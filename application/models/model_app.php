...
class Model_app extends CI_Model{
    ...
    function getLapPenjualan($tgl_awal,$tgl_akhir){
        return $this->db->query("SELECT *,(SELECT sum(total_harga) FROM `tbl_penjualan_header` WHERE tanggal_penjualan='2016-05-17') AS total_all from tbl_penjualan_header a
                left join tbl_pelanggan b on a.kd_pelanggan=b.kd_pelanggan
                left join tbl_pegawai c on a.kd_pegawai=c.kd_pegawai
                group by a.kd_penjualan having tanggal_penjualan between '$tgl_awal' and '$tgl_akhir'
                ")->result();
    }
